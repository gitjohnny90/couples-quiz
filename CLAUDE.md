...
- **Predict Your Partner**: `src/pages/PredictPartnerPage.jsx` — 4 series × 4 packs × 3 questions (48 total). Each player answers for themselves AND predicts partner's answer. Data stored in `responses` table with `pack_id: 'predict-pack-N'`, per-player rows. JSONB `answers` contains `{ responses: [{ ownAnswer, prediction }], partnerPredictionMarks: [bool|null], completedAt }`. During reveal, each player marks whether partner's predictions about them were correct (checkmark/X). Score labels from 0/6 to 6/6. Questions in `src/data/predictPartnerQuestions.js`. Uses realtime + 5s polling.
