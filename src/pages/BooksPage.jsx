...
const MAX_VETOES = 2
const { getVetoInfo, saveVeto } = createVetoSystem('book_vetoes')
...
export default function BooksPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`shared-items-books-${sessionId}`)
  ...
  const fetchData = async () => {
    const [{ data: sessionData }, { data: itemData }] = await Promise.all([
      ...
      supabase.from('shared_items').select('*').eq('session_id', sessionId).eq('type', 'book').order('created_at', { ascending: false }),
    ])
  ...
  // Fun stats
  const totalRead = finishedItems.length
        ...
        {tab === 'list' && (
          <div>
            ...
            {!showAdd ? (
              ...
              <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="glass" style={{ padding: 16, marginBottom: 16 }}>
                ...
                <select value={newGenre} onChange={(e) => setNewGenre(e.target.value)} style={{ marginBottom: 12 }}>
                  ...
                  {bookGenres.map((g) => <option key={g.id} value={g.name}>{g.emoji} {g.name}</option>)}
                </select>
            ...
            {currentItems.length > 0 && (
              <div style={{ marginBottom: 16 }}>
                ...
                {currentItems.map((item) => (
                  <BookCard key={item.id} item={item} session={session} playerId={playerId}
            ...
            {abandonedItems.length > 0 && (
              <div style={{ marginBottom: 16 }}>
                <h3 style={{ fontFamily: 'var(--font-hand)', fontSize: '1.2rem', color: 'var(--text-light)', marginBottom: 8 }}>
        ...
        {tab === 'pick' && (
          <div style={{ textAlign: 'center' }}>
            <div className="sticky-note" style={{ padding: 22, marginBottom: 18, textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                we'll pick from your "want to read" list!
              </p>
            </div>
            ...
            {wantItems.length === 0 ? (
              <div className="glass" style={{ padding: 20, textAlign: 'center' }}>
                <p style={{ fontFamily: 'var(--font-hand)', fontSize: '1.1rem', color: 'var(--text-secondary)' }}>
                  add some books to your list first!
                </p>
                    ...
                    >
                      {pickRevealed ? (
                        <>
                          ...
                          <p style={{ fontFamily: 'var(--font-hand)', fontSize: '1.8rem', fontWeight: 700, color: 'var(--accent-sage)', lineHeight: 1.2, marginBottom: 4 }}>
                            {currentPick.title}
                          </p>
                          {currentPick.genre && (
        ...
        {tab === 'wheel' && (
          <div>
            ...
            {selectedGenre && (
              <SpinningWheel
                key={selectedGenre}
                items={bookGenres.find((g) => g.id === selectedGenre)?.titles || []}
...
// ===== Book Card Component =====
function BookCard({ item, session, playerId, onStatusChange, onRate, onDelete }) {
  const [expanded, setExpanded] = useState(false)
  const [confirmDelete, setConfirmDelete] = useState(false)

  const statusOptions = [
    { value: 'want', label: 'want to read', color: 'var(--accent-mustard)' },
    { value: 'current', label: 'reading now', color: 'var(--accent-coral)' },
    { value: 'finished', label: 'finished', color: 'var(--accent-sage)' },
    { value: 'abandoned', label: 'abandoned', color: 'var(--text-light)' },
  ]
